  ...
  app.innerHTML = `
    <div class="min-h-screen flex flex-col">
      <div class="bg-brand-800 text-white px-6 pt-12 pb-10">
        <div class="text-sm opacity-80 mb-1">Service Portal</div>
        <h1 class="text-2xl font-bold tracking-tight">Callison Electric & HVAC</h1>
        <p class="text-brand-100 text-sm mt-2">Schedule service • Track jobs • Pay bills</p>
      </div>
      <div class="flex-1 px-6 py-8 -mt-4">
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
          <h2 class="font-semibold text-lg mb-4">Sign in</h2>
          <form id="loginForm" class="space-y-4">
            <div>
              <label class="block text-xs font-medium text-slate-500 mb-1">Email</label>
              <input name="email" type="email" required class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 outline-none" placeholder="you@email.com" />
            </div>
            <div>
              <label class="block text-xs font-medium text-slate-500 mb-1">Password</label>
              <input name="password" type="password" required class="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 outline-none" />
            </div>
            <button type="submit" class="w-full bg-brand-600 hover:bg-brand-700 text-white font-semibold py-3 rounded-xl">Sign In</button>
          </form>
          <div class="mt-5 pt-5 border-t border-slate-100 text-center">
            <p class="text-sm text-slate-500 mb-2">New customer?</p>
            <button id="showRegister" class="text-brand-600 font-medium text-sm">Create an account</button>
          </div>
  ...
  $('#app').innerHTML = `
    <div class="min-h-screen flex flex-col">
      <div class="bg-brand-800 text-white px-6 pt-10 pb-8">
        <button id="backLogin" class="text-brand-200 text-sm mb-3">← Back</button>
        <h1 class="text-xl font-bold">Create Customer Account</h1>
        <p class="text-brand-100 text-sm mt-1">Schedule service and pay bills online</p>
      </div>
      <div class="px-6 py-6">
        <form id="regForm" class="space-y-3">
          ...
          <div><label class="text-xs font-medium text-slate-500">Phone</label>
            <input name="phone" type="tel" class="w-full border rounded-xl px-3 py-2.5 text-sm mt-1" placeholder="555-123-4567" /></div>
          ...
          <button class="w-full bg-brand-600 text-white font-semibold py-3 rounded-xl mt-2">Create Account</button>
        </form>
      </div>
    </div>`;
    ...
    <div class="min-h-screen flex flex-col pb-20">
      <header class="bg-brand-800 text-white px-4 py-3 sticky top-0 z-20">
        <div class="flex items-center justify-between">
          <div>
            <div class="text-xs text-brand-200">Callison Electric & HVAC</div>
            <div class="font-semibold text-sm">${title}</div>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-xs bg-brand-700 px-2 py-1 rounded-lg">${currentUser.name.split(' ')[0]}</span>
            <button id="logoutBtn" class="text-xs text-brand-200 underline">Log out</button>
          </div>
        </div>
      </header>
      ...
      <nav class="fixed bottom-0 left-0 right-0 max-w-lg mx-auto bg-white border-t border-slate-200 flex z-20">
        ${nav.map(t => `
          <button data-tab="${t.id}" class="tab-btn flex-1 py-3 text-xs font-medium ${tabs===t.id ? 'text-brand-600' : 'text-slate-400'}">
            ${t.label}
          </button>`).join('')}
      </nav>
    </div>`;
  ...
  const content = `
    <div class="space-y-4">
      <div class="bg-gradient-to-br from-brand-600 to-brand-800 text-white rounded-2xl p-5">
        ...
        <div class="text-xl font-bold mt-0.5">${currentUser.name.split(' ')[0]}</div>
        <p class="text-brand-100 text-sm mt-2">Need service? Schedule in under a minute.</p>
        <button data-go="schedule" class="mt-4 bg-white text-brand-800 font-semibold text-sm px-4 py-2 rounded-xl">Schedule Service</button>
      </div>
      ${unpaid.length ? `
  ...
  const content = `
    <div class="space-y-4">
      ...
      <form id="schedForm" class="space-y-3">
        ...
        <button type="submit" class="w-full bg-brand-600 text-white font-semibold py-3 rounded-xl">Submit Request</button>
      </form>
    </div>`;
    ...
    <div class="space-y-5">
      ...
      ${unpaid.map(inv => `
        <div class="border border-red-100 bg-red-50/50 rounded-2xl p-4">
          ...
          <button data-pay="${inv.id}" class="pay-btn mt-3 w-full bg-brand-600 text-white font-semibold py-2.5 rounded-xl text-sm">
        ...
        <div class="border border-slate-200 rounded-2xl p-4 shadow-sm">
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            ${wo.status==='scheduled'||wo.status==='in_progress'?`
              <button class="onway-btn flex-1 bg-violet-600 text-white text-sm font-medium py-2.5 rounded-xl" data-id="${wo.id}">On the Way + SMS</button>`:''}
            ${wo.status!=='completed'?`
              <button class="complete-btn flex-1 bg-emerald-600 text-white text-sm font-medium py-2.5 rounded-xl" data-id="${wo.id}">Complete + SMS</button>`:''}
          </div>
          <div class="mt-2">
      ...
      reqs.map(r => `
        <div class="border rounded-2xl p-4">
          ...
          <div class="mt-3 flex gap-2 items-center">
            ...
            <button class="convert-btn bg-brand-600 text-white text-sm px-3 py-1.5 rounded-lg font-medium" data-id="${r.id}">Convert</button>
          </div>
        </div>`).join('')}
    </div>`;
